...
class FlywindBuilder implements Builder {
  ...
  Map<String, List<String>> get buildExtensions => {
    'lib/flywind.g.yaml': ['lib/tokens/tokens.dart'],
  };

  @override
  Future<void> build(BuildStep buildStep) async {
    // Get the input asset (flywind.yaml)
    final inputId = buildStep.inputId;
    log.info('Processing input: ${inputId.path}');

    if (!inputId.path.endsWith('lib/flywind.g.yaml')) {
      log.warning('Expected lib/flywind.g.yaml input, got: ${inputId.path}');
      return;
    }

    final configContent = await buildStep.readAsString(inputId);
    log.info('Found flywind.yaml, generating tokens...');
    ...
    final tokensAsset = AssetId(inputId.package, 'lib/tokens/tokens.dart');
    await buildStep.writeAsString(tokensAsset, tokensContent);

    log.info('Generated tokens file at lib/tokens/tokens.dart');
  }
  ...
  String _generateTokensFile(Map<dynamic, dynamic> config) {
    ...
    for (final entry in tokens.entries) {
      ...
      final type = tokenConfig['type'] as String;
      final canBeConst = _canBeConst(type);
      final constKeyword = canBeConst ? 'const' : '';

      buffer.writeln('  /// $className instance');
      buffer.writeln(
        '  static $constKeyword $className ${_getFieldName(tokenName)} = $className.default${className.replaceAll('Fly', '')};',
      );
      buffer.writeln();
    }
  ...
  String _generateTokenClass(String tokenName, Map<dynamic, dynamic> config) {
    ...
    final constKeyword = canBeConst ? 'const' : '';
    buffer.writeln(
      '  static $constKeyword $className default${className.replaceAll('Fly', '')} = $className(',
    );
  ...
  /// Check if a type can be const
  bool _canBeConst(String type) {
    return type == 'Color' ||
        type == 'double' ||
        type == 'int' ||
        type == 'String';
