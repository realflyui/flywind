// GENERATED FILE - DO NOT EDIT MANUALLY
// To regenerate, run: dart cli/generate_defaults.dart

import '../core/token.dart';

/// Blur effect values
class FlyBlurToken implements FlyToken<double> {
  const FlyBlurToken({
    required this.xs,
    required this.sm,
    required this.md,
    required this.lg,
    required this.xl,
    required this.xl2,
    required this.xl3,
    this.extras = const {},
  });

  /// xs Blur effect values
  final double xs;
  /// sm Blur effect values
  final double sm;
  /// md Blur effect values
  final double md;
  /// lg Blur effect values
  final double lg;
  /// xl Blur effect values
  final double xl;
  /// 2xl Blur effect values
  final double xl2;
  /// 3xl Blur effect values
  final double xl3;

  /// Additional custom values
  final Map<String, double> extras;

  /// All values in a single map for easier iteration
  Map<String, double> get _allValues => {
    'xs': xs,
    'sm': sm,
    'md': md,
    'lg': lg,
    'xl': xl,
    'xl2': xl2,
    'xl3': xl3,
    ...extras,
  };

  /// Access value by key (canonical or extra)
  @override
  double? operator[](String key) => _allValues[key];

  /// Get all available keys (canonical + extras)
  @override
  Iterable<String> get keys => _allValues.keys;

  /// Put a new value for the given key
  @override
  FlyBlurToken put(String key, double value) {
    switch (key) {
      case 'xs':
        return copyWith(xs: value);
      case 'sm':
        return copyWith(sm: value);
      case 'md':
        return copyWith(md: value);
      case 'lg':
        return copyWith(lg: value);
      case 'xl':
        return copyWith(xl: value);
      case 'xl2':
        return copyWith(xl2: value);
      case 'xl3':
        return copyWith(xl3: value);
      default:
        final newExtras = Map<String, double>.from(extras);
        newExtras[key] = value;
        return copyWith(extras: newExtras);
    }
  }

  /// Merge another token into this one (right side wins)
  @override
  FlyBlurToken merge(FlyToken<double> other) {
    if (other is! FlyBlurToken) return this;
    
    return copyWith(
      xs: other.xs,
      sm: other.sm,
      md: other.md,
      lg: other.lg,
      xl: other.xl,
      xl2: other.xl2,
      xl3: other.xl3,
      extras: {...extras, ...other.extras},
    );
  }

  /// Create a copy with updated values
  FlyBlurToken copyWith({
    double? xs,
    double? sm,
    double? md,
    double? lg,
    double? xl,
    double? xl2,
    double? xl3,
    Map<String, double>? extras,
  }) {
    return FlyBlurToken(
      xs: xs ?? this.xs,
      sm: sm ?? this.sm,
      md: md ?? this.md,
      lg: lg ?? this.lg,
      xl: xl ?? this.xl,
      xl2: xl2 ?? this.xl2,
      xl3: xl3 ?? this.xl3,
      extras: extras ?? this.extras,
    );
  }

  /// Linear interpolation between two tokens
  FlyBlurToken lerp(FlyBlurToken other, double t) {
    final result = <String, double>{};
    final allKeys = {..._allValues.keys, ...other._allValues.keys};
    
    for (final key in allKeys) {
      final valueA = _allValues[key];
      final valueB = other._allValues[key];
      final numA = valueA ?? 0.0;
      final numB = valueB ?? 0.0;
      result[key] = numA + (numB - numA) * t;
    }
    
    return FlyBlurToken(
      xs: result['xs']!,
      sm: result['sm']!,
      md: result['md']!,
      lg: result['lg']!,
      xl: result['xl']!,
      xl2: result['xl2']!,
      xl3: result['xl3']!,
      extras: Map.fromEntries(result.entries.where((e) => !['xs', 'sm', 'md', 'lg', 'xl', 'xl2', 'xl3'].contains(e.key))),
    );
  }

  /// Create default values
  static FlyBlurToken defaultBlur() {
    return const FlyBlurToken(
      xs: 4.0,
      sm: 8.0,
      md: 12.0,
      lg: 16.0,
      xl: 24.0,
      xl2: 40.0,
      xl3: 64.0,
    );
  }

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is FlyBlurToken && _allValues.toString() == other._allValues.toString();
  }

  @override
  int get hashCode => _allValues.hashCode;
}
